...
CCardControl::CCardControl()
{
	...
	m_PngHand_R.LoadImage(hResInstance,TEXT("HAND_R"));
	m_PngHand_L.LoadImage(hResInstance,TEXT("HAND_L"));
...
VOID CCardControl::DrawCardControl(CDC * pDC)
{
	...
	for (WORD i=0;i<m_wCardCount;i++)
	{
		...
		if(m_blGameEnd)
		{
			m_ImageCard.TransDrawImage(pDC,OriginPoint.x+nXDrawPos+15,OriginPoint.y+nYDrawPos+12,m_CardSize.cx,m_CardSize.cy,nXImagePos,nYImagePos,RGB(255,0,255));
		}
		else
		{
			//绘画扑克
			if(1==i)
			{
				m_ImageCard.TransDrawImage(pDC,OriginPoint.x+nXDrawPos+m_MovePoint.x+25,OriginPoint.y+nYDrawPos+12+m_MovePoint.y,m_CardSize.cx,m_CardSize.cy,nXImagePos,nYImagePos,RGB(255,0,255));
				...
				m_PngHand_R.DrawImage(pDC,OriginPoint.x+nXDrawPos+m_MovePoint.x+67,OriginPoint.y+nYDrawPos+34+m_MovePoint.y);
				
			}
			else
			{
				m_ImageCard.TransDrawImage(pDC,OriginPoint.x+nXDrawPos+25,OriginPoint.y+nYDrawPos+12,m_CardSize.cx,m_CardSize.cy,nXImagePos,nYImagePos,RGB(255,0,255));
				m_PngHand_L.DrawImage(pDC,OriginPoint.x-14,OriginPoint.y+nYDrawPos-15);
			}
		}
		//绘画选择
