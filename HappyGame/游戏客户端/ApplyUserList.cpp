...
BOOL CApplyUser::OnInitDialog()
{
	...
	m_AppyUserList.SetTextColor(RGB(232, 245, 130));

	//插入列表
	m_AppyUserList.InsertColumn(0,TEXT("申请玩家"),LVCFMT_LEFT,70);

	m_AppyUserList.InsertColumn(1,TEXT("欢乐豆"),LVCFMT_LEFT,70);

	m_AppyUserList.InsertColumn(2,TEXT("欢乐豆"),LVCFMT_LEFT,30);
...
void CApplyUser::InserUser(tagApplyUser & ApplyUser)
{
	...
	{
		//游戏玩家
		INT nItemIndex=m_AppyUserList.InsertItem(0,ApplyUser.strUserName);

		//玩家金币
		_sntprintf(szBuffer,CountArray(szBuffer),TEXT("%I64d"),ApplyUser.lUserScore);
		m_AppyUserList.SetItem(0,wColumnCount++,LVIF_TEXT,szBuffer,0,0,0,0);

		m_AppyUserList.SetItem(0,wColumnCount++,LVIF_TEXT,TEXT("抢"),0,0,0,0);
	}
	else
	{
		//游戏玩家
		...
		m_AppyUserList.SetItem(nItemIndex,wColumnCount++,LVIF_TEXT,TEXT(""),0,0,0,0);
	}
