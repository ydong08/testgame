...
bool CGameClientDlg::OnSubGameEnd( void * pBuffer, WORD wDataSize)
{
	...
	//swprintf(szBuffer,CountArray(szBuffer),TEXT("本方：%I64d"),pGameEnd->lUserScore);
	//m_pIStringMessage->InsertCustomString(szBuffer,RGB(255,255,255));

	//swprintf(szBuffer,CountArray(szBuffer),TEXT("庄家：%I64d"),pGameEnd->lBankerScore);
	//m_pIStringMessage->InsertCustomString(szBuffer,RGB(255,255,255));
...
void CGameClientDlg::UpdateButtonContron()
{
	...
	if (bEnablePlaceJetton==true && !m_GameClientView.m_bCheckOverBtn)
	{
		...
		m_GameClientView.m_btJetton5000000.EnableWindow((lUserMaxJetton>5000000 && m_lBankerScore>5000000)?TRUE:FALSE);  //要求修改为5KW
	}
	else
	{
		//设置光标
...
LRESULT CGameClientDlg::OnPlaceJetton(WPARAM wParam, LPARAM lParam)
{
	...
	//CString str;
	//str.Format(TEXT("下注区域:%d"),cbJettonArea);
	//AfxMessageBox(str);
...
LRESULT  CGameClientDlg::OnOpenCard(WPARAM wParam, LPARAM lParam)
{
	if (GetGameStatus()==GS_GAME_END)
	{
		m_pIStringMessage->InsertCustomString(TEXT("［系统提示］开牌时间不能切换腾牌模式！"),RGB(255,255,255));
		return 1;

	}
	m_GameClientView.m_blAutoOpenCard = false;
	m_pIStringMessage->InsertCustomString(TEXT("［系统提示］您选择了手工腾牌模式，发牌后您可以使用鼠标拖动扑克！"),RGB(255,255,255));
	m_GameClientView.m_btOpenCard.EnableWindow(false);
	m_GameClientView.m_btAutoOpenCard.EnableWindow(true);
...
LRESULT  CGameClientDlg::OnAutoOpenCard(WPARAM wParam, LPARAM lParam)
{
	...
	m_pIStringMessage->InsertCustomString(TEXT("［系统提示］您选择了自动腾牌模式，开牌后系统将自动开出各家的牌！"),RGB(255,255,255));
	m_GameClientView.m_btOpenCard.EnableWindow(true);
	m_GameClientView.m_btAutoOpenCard.EnableWindow(false);
...
LRESULT CGameClientDlg::OnQiangBanker(WPARAM wParam,LPARAM lParam)
{
	...
	if (pMeUserData->GetUserScore() < m_lQiangCondition) 
	{
		m_pIStringMessage->InsertCustomString(TEXT("您的欢乐豆低于抢庄条件，抢庄失败！"),RGB(255,255,0));
		return true;
	}
	...
	{
		m_pIStringMessage->InsertCustomString(TEXT("您已经是庄家，申请失败！"),RGB(255,255,0));
		return true;
	}
	...
	if (bApplyBanker)
	{
		...
		CString szMessage;
		szMessage.Format(TEXT("确定要支付%I64d 欢乐豆抢庄吗？"),m_lQiangScore);
		if(IDYES==info.ShowMessageBox(TEXT("指导费"),szMessage,MB_YESNO))
		{
			//发送消息
