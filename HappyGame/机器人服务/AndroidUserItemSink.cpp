			...
			if(rand() % 100 < m_nApplyRand)
			{
				...
				if ( m_bRobotBanker)// && m_stlApplyBanker < m_nRobotApplyBanker)
				{
					...
					if (pIUserItemBanker->GetUserScore() >= m_lBankerCondition) 
					{
						...
						/*m_nBankerCount = 0;
						m_stlApplyBanker++;*/
						m_pIAndroidUserItem->SetGameTimer(IDI_REQUEST_BANKER, (rand() % 30) + 1);
					}
				}
			}
			return false;
	...
	default:
		{
			...
			{
				//if(m_bIsAddScore == true)return false;
				//srand(GetTickCount());

				if (rand()%100 > 50)return false;  //随机不下
...
bool  CAndroidUserItemSink::OnEventGameMessage(WORD wSubCmdID, void * pBuffer, WORD wDataSize)
{
	...
	case SUB_S_ROBOT_BANKER:
		{
			srand(NULL);
			m_pIAndroidUserItem->SetGameTimer(IDI_REQUEST_BANKER, (rand() % 8) + 1);
			return true;
		}
...
bool  CAndroidUserItemSink::OnEventSceneMessage(BYTE cbGameStatus, bool bLookonOther, void * pData, WORD wDataSize)
{
	...
	case GAME_STATUS_FREE:			//空闲状态
		{
			...
			m_dwServerID=pStatusFree->dwServerID;

			ReadConfigInformation(m_RobotInfo.szCfgFileName, m_dwServerID, true);
		
			//CString StrScore;
			//StrScore.Format(TEXT("%I64d  %I64d"),m_lMaxTabelScore,m_lMaxTabelAddScore);
			//CTraceService::TraceString(StrScore,TraceLevel_Normal);
			...
			//if (pStatusFree->wBankerUser == INVALID_CHAIR)
			{
				if (m_bRobotBanker && m_nRobotWaitBanker == 0  && m_stlApplyBanker < m_nRobotApplyBanker)
				{
					...
					if (pIUserItemBanker->GetUserScore() > m_lBankerCondition) 
					{
						...
						m_pIAndroidUserItem->SetGameTimer(IDI_REQUEST_BANKER, cbTime+rand()%3);
					}
				}
			}
	...
	case GS_GAME_END:		//结束状态
		...
		m_dwServerID=pStatusPlay->dwServerID;
...
bool CAndroidUserItemSink::OnSubGameFree(const void * pBuffer, WORD wDataSize)
{
	//读取配置
	if (m_bRefreshCfg)
		ReadConfigInformation(m_RobotInfo.szCfgFileName, m_dwServerID, false);
	...
	{
		m_nBankerCount++;
		if ( m_nBankerCount >= (m_nRobotBankerCount+rand()%7) )
		{
			//机器人走庄
			m_nBankerCount = 0;
	...
	if (m_wCurrentBanker != m_pIAndroidUserItem->GetChairID() || bMeGiveUp)
	{
		...
		m_pIAndroidUserItem->SetGameTimer(IDI_CHECK_BANKER, 1);
	}
...
{	
	...
	{
		int nRandRage = int( nTimeGrid * i / (3000*sqrt((double)m_nChipTime)) ) + 1;		//波动范围
...
bool CAndroidUserItemSink::OnSubUserCancelBanker(const void * pBuffer, WORD wDataSize)
{
	...
	//自己判断
	if (pCancelBanker->dwCancelUser==m_pIAndroidUserItem->GetChairID()) 
		m_bMeApplyBanker=false;
...
VOID CAndroidUserItemSink::ReadApplyRandInfo()
{
	//设置文件名
	...
	_sntprintf(szConfigFileName, sizeof(szConfigFileName), _T("%s\\%s"), szPath,TEXT("SupremeConfig.ini"));
	m_nApplyRand = GetPrivateProfileInt(m_szRoomName, _T("RobotRandIndex"), 10, szConfigFileName);
}


//读取配置
void CAndroidUserItemSink::ReadConfigInformation(TCHAR szFileName[], DWORD dwServerID, bool bReadFresh)
{
	//设置文件名
	TCHAR szPath[MAX_PATH] = TEXT("");
	TCHAR szConfigFileName[MAX_PATH] = TEXT("");
	...
	TCHAR szRoomName[MAX_PATH] = TEXT("");

	_sntprintf(szRoomName, sizeof(szRoomName), _T("%d"), dwServerID);
