	...
	case ACK_SET_WIN_AREA:
		{
			...
			{  				
				...
				else if(m_cbControlStyle==CS_BET_AREA)
				{
					...
					if(m_bWinArea[0]) strShun=TEXT(" 顺门 ");
					if(m_bWinArea[1]) strDui=TEXT(" 对门 ");
					if(m_bWinArea[2]) strDao=TEXT(" 倒门 ");
					if(!m_bWinArea[0]&&!m_bWinArea[1]&&!m_bWinArea[2]) strShun=TEXT(" 庄通吃 ");
...
void CClientControlItemSinkDlg::ResetUserBet()
{
	...
	SetDlgItemText(IDC_ST_AREA1,TEXT("左横:-----"));
	SetDlgItemText(IDC_ST_AREA2,TEXT("右横:-----"));
	SetDlgItemText(IDC_ST_AREA3,TEXT("顺门:-----"));
	SetDlgItemText(IDC_ST_AREA4,TEXT("倒门:-----"));
	SetDlgItemText(IDC_ST_AREA5,TEXT("左角:-----"));
	SetDlgItemText(IDC_ST_AREA6,TEXT("右角:-----"));
	SetDlgItemText(IDC_STATIC_Area111,TEXT("对门:-----"));

	SetDlgItemText(IDC_ST_AREA7,TEXT("左横:-----"));
	SetDlgItemText(IDC_ST_AREA8,TEXT("右横:-----"));
	SetDlgItemText(IDC_ST_AREA9,TEXT("顺门:-----"));
	SetDlgItemText(IDC_ST_AREA10,TEXT("倒门:-----"));
	SetDlgItemText(IDC_ST_AREA11,TEXT("左角:-----"));
	SetDlgItemText(IDC_ST_AREA12,TEXT("右角:-----"));
	SetDlgItemText(IDC_STATIC_AREE2222,TEXT("对门:-----"));
...
void CClientControlItemSinkDlg::SetUserBetScore(BYTE cbArea,LONGLONG lScore)
{
	...
	case 1: 
		{
			strPrint.Format(TEXT("右角:%I64d"),m_lAllUserBet[cbArea]);
			SetDlgItemText(IDC_ST_AREA6,strPrint);
			break;
		}
	case 2: 
		{
			strPrint.Format(TEXT("右横:%I64d"),m_lAllUserBet[cbArea]);//
	...
	case 5: 
		{
			strPrint.Format(TEXT("左角:%I64d"),m_lAllUserBet[cbArea]);
			SetDlgItemText(IDC_ST_AREA5,strPrint);
			break;
		}	
	case 6: 
		{
			strPrint.Format(TEXT("左横:%I64d"),m_lAllUserBet[cbArea]);
...
void CClientControlItemSinkDlg::SetUserGameScore(BYTE cbArea,LONGLONG lScore)
{
	...
	case 1: 
		{
			strPrint.Format(TEXT("右角:%I64d"),lScore);
			SetDlgItemText(IDC_ST_AREA12,strPrint);
			break;
		}
	case 2: 
		{
			strPrint.Format(TEXT("右横:%I64d"),lScore);
	...
	case 5: 
		{
			strPrint.Format(TEXT("左角:%I64d"),lScore);
			SetDlgItemText(IDC_ST_AREA11,strPrint);
			break;
		}
	case 6: 
		{
			strPrint.Format(TEXT("左横:%I64d"),lScore);
...
BOOL CClientControlItemSinkDlg::OnInitDialog()
{
	...
	SetDlgItemText(IDC_BT_CURSET,TEXT("本局"));
