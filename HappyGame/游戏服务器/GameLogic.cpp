...
BYTE CGameLogic::GetCardType(const BYTE cbCardData[], BYTE cbCardCount)
{
	...
	if ((0+2)==(cbFistCardColor+cbSecondCardColor) && cbFirstCardValue==cbSecondCardValue && 10==cbFirstCardValue) return CT_SPECIAL_9;
	if ((0+2)==(cbFistCardColor+cbSecondCardColor) && cbFirstCardValue==cbSecondCardValue && 7==cbFirstCardValue) return CT_SPECIAL_10;
	if ((0+2)==(cbFistCardColor+cbSecondCardColor) && cbFirstCardValue==cbSecondCardValue && 6==cbFirstCardValue) return CT_SPECIAL_11;
	if ((1+3)==(cbFistCardColor+cbSecondCardColor) && cbFirstCardValue==cbSecondCardValue && 11==cbFirstCardValue) return CT_SPECIAL_12;
...
//获取四付牌大小排列
void CGameLogic::GetCardOrder(const BYTE cbCardArray[][2], BYTE cbArrayCount, BYTE cbOrder[], BYTE &cbMinIndex, BYTE &cbMaxIndex)
{
	BYTE cbMinOrder,cbMaxOrder;
	cbMinIndex = 0xFF;
	cbMaxIndex = 0xFF;

	for(BYTE i=0;i<cbArrayCount;i++)
	{
		cbOrder[i] = 0;
		for(BYTE j=0;j<cbArrayCount;j++)
		{
			if(i != j && CompareCard(cbCardArray[i],2,cbCardArray[j],2) == 1)
			{
				cbOrder[i] ++;
			}
		}
		if(cbMinIndex == 0xFF || cbOrder[i] > cbMinOrder || cbOrder[i] == cbMinOrder && rand()%2==0)
		{
			cbMinOrder = cbOrder[i];
			cbMinIndex = i;
		}
		if(cbMaxIndex == 0xFF || cbOrder[i] < cbMaxOrder || cbOrder[i] == cbMaxOrder && rand()%2==0)
		{
			cbMaxOrder = cbOrder[i];
			cbMaxIndex = i;
		}
	}
}
