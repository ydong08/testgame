...
protected:
	...
	//个人下注
protected:
	LONGLONG						m_lUserJettonScore[AREA_COUNT][GAME_PLAYER];//个人总注
...
protected:
	...
	int								m_nBankerTimeAdd;						//增加次数 (欢乐豆大于其他玩家时)
	LONGLONG						m_lExtraBankerScore;					//额外条件 (大于此值时可以无视条件额外坐庄)
	int								m_nExtraBankerTime;						//额外次数
	int                             m_nQiangTimeAdd;                        //抢庄额外增加的次数
	LONGLONG                        m_lQiangCondition;                      //抢庄限制
	WORD                            m_wQiangBanker;                           //抢庄着
	...
	tagPunishUser				m_PunishUser[128];						//惩罚用户
	int							m_nPunishUserCount;						//惩罚用户数量
